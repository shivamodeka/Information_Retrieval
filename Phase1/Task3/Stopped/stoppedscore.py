...
def generate_scores(query, stopwords): #generates scores for each documents for the query
    data = query.split()
    newdata = []
    for word in data:
        if word not in stopwords:
            newdata.append(word) #remove stopwords from query
    data = newdata
...
if __name__ == "__main__":
    ...
    for query in queries:
        BM25_query_result, TFIDF_query_result, Smooth_query_result = generate_scores(query, common_words) #generate scores
        writeScores(BM25_query_result, Qid, query_id, " BM25_QStopped_CaseFolded_System", "BM25 - ", "Stopped-BM25") #write scores
        writeScores(TFIDF_query_result, Qid, query_id, " TFIDF_QStopped_CaseFolded_System", "TFID - ", "Stopped-TFIDF")
        writeScores(Smooth_query_result, Qid, query_id, " Smooth_QStopped_CaseFolded_System", "Smooth - ", "Stopped-SMOOTH")
        query_id += 1
