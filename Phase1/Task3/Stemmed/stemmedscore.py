...
from makestemmedcorpus import makecorpus
import re

makecorpus() #make corpus for stemmed text file
...
def generate_scores(query): #generating qf dictionary that holds frequency of query terms for BM25
    data = query.split()
...
def getQueries():
    f = open ("cacm_stem.query.txt", "r") #reading query files
    QueryInput = f.read()
...
def writeScores(ScoreDict, Qid, count, system_name, fname, subfolder): #writing top 100 scores
    filename = os.path.join(os.getcwd(), "Query Results", subfolder, fname + Qid[count] + ".txt")
...
if __name__ == "__main__":
    ...
    for query in queries:
        BM25_query_result, TFIDF_query_result, Smooth_query_result = generate_scores(query) #generate scores
        writeScores(BM25_query_result, Qid, query_id, " BM25_Stemmed_CaseFolded_System", "BM25 - ", "STEM-BM25") #write scores
        writeScores(TFIDF_query_result, Qid, query_id, " TFIDF_Stemmed_CaseFolded_System", "TFID - ", "STEM-TFIDF")
        writeScores(Smooth_query_result, Qid, query_id, " Smooth_Stemmed_CaseFolded_System", "Smooth - ", "STEM-SMOOTH")
        query_id += 1
